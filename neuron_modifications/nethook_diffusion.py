...
class Trace(contextlib.AbstractContextManager):
    ...
    def __init__(
        ...
        # Layer to be edited inside the trace function
        #print(f'Layer inside the trace function.. : {self.layer}')
        ...
        def retain_hook(m, inputs, output):
            ...
            # Perform the replacement operation here # TODO : Push this to a function later on
            # Indexes {'0': uncon_prompt1, '1': uncon_prompt2, '2': con_prompt1, '3': con_prompt2}
            # Replace the corrupted embedding with the uncorrupted one for the state [position-2]
            """ Output replacement operation """

            # print(m.weight.shape) # (320 x 768)
            # print(inputs.shape)
            # input_to_ca = inputs[0] # (2, 77, 768)

            # # print(output.shape) # (2, 77, 320) # The first position contains the compositional prompt

            # #print(type(input_to_ca))
            weight = torch.tensor(list(m.parameters())[0])
            #print(weight.shape)

            # Modified Output (input @ weight)
            ...
            #     output[1, :, modification_info['indices']] = modification_info['values']
...
class TraceDict(OrderedDict, contextlib.AbstractContextManager):
    ...
    def __init__(
        ...
        modification_layers = [16, 17, 18, 19],
        num_of_neurons = 100,
        modification_info = None, 
        retain_output=True,
        retain_input=False,
        ...
        if type(layers) == list:
            ...
            for is_last, layer in flag_last_unseen(layers):
                if c in modification_layers:
                    tmp = {
                        'indices': modification_info[c][num_of_neurons]['indices'].to('cuda'),
                        'values': modification_info[c][num_of_neurons]['values'].type(torch.float).to('cuda')
                        }
                else:
                    tmp = None
                ...
                self[layer] = Trace(
                    ...
                    modification_flag=c in modification_layers,
                    modification_info=tmp, 
